/**
 * Physical AI Textbook — RAG Chatbot Widget
 * Connects to FastAPI /api/chat backend with graceful offline fallback.
 * Personalized per user profile from Better-Auth context.
 */
...
// ── Fallback KB (used when backend is offline) ────────────────────────────
...
const FALLBACK_KB = [
  ...
  },
  {
    ...
    ans: 'Module 6 — Capstone: Build an autonomous humanoid robot integrating ROS 2, VLA models, and NVIDIA Isaac. Full specs in docs/capstone-autonomous-humanoid.md.',
  },
  {
...
function fallbackAnswer(query: string): { text: string; sources: [] } {
  ...
  let best = 0;
  let answer = "I'm your Physical AI textbook assistant! Ask about ROS 2, NVIDIA Isaac, VLA models, simulation, sensors, or the capstone project.";

  for (const entry of FALLBACK_KB) {
    const score = entry.kw.reduce((s, k) => s + (q.includes(k) ? k.split(' ').length * 2 : 0), 0);
    if (score > best) {
      best = score;
      answer = entry.ans;
    }
...
export default function ChatWidget(): React.JSX.Element {
  ...
  const sendMessage = useCallback(async (text?: string) => {
    ...
    try {
      let result: { answer: string; sources: any[]; mode: string };
      ...
      const errMsg: Message = {
        id: errId,
        role: 'bot',
        text: "I couldn't reach the AI backend. Check that `uvicorn main:app` is running on port 8000.",
        time: getTime(),
        mode: 'fallback',
      };
  ...
  const statusLabel = connStatus === 'online' ? 'AI Online · RAG' : connStatus === 'checking' ? 'Connecting…' : 'Offline · Local KB';
  const statusColor = connStatus === 'online' ? '#22c55e' : connStatus === 'checking' ? '#f59e0b' : '#94a3b8';
  const suggestions = urduMode ? SUGGESTIONS_URDU : SUGGESTIONS_DEFAULT;
      ...
      >
        ...
        <div className={styles.panelHeader}>
          <div className={styles.headerLeft}>
            ...
            <div>
              <div className={styles.botName}>
                PhysAI Tutor
                {user && <span style={{ fontSize: '0.65rem', opacity: 0.7, marginLeft: '0.4rem' }}>
                  · {user.name.split(' ')[0]}
                </span>}
              </div>
          ...
          <div className={styles.headerActions}>
            ...
            <button className={styles.clearBtn} onClick={clearChat} title="Clear conversation" aria-label="Clear">
        ...
        <div className={styles.panelFooter} aria-hidden="true">
          {connStatus === 'online'
            ? '⚡ Powered by OpenAI GPT-4o-mini + ChromaDB RAG'
